...
export default function FollowupNotifier() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      const { data } = await supabase.from('lead_followups').select('id,title,due_at,lead_id').eq('assigned_to', user.id).eq('status', 'pendente').lte('due_at', now).order('due_at', { ascending: true }).limit(20)
      const due = (data ?? []) as Due[]
      if (!mounted) return
      setNotice(due)
      const key = `cadena-followups-${user.id}`
      const sent = JSON.parse(localStorage.getItem(key) || '[]') as string[]
      const fresh = due.filter(item => !sent.includes(item.id))
      if (fresh.length && 'Notification' in window) {
        if (Notification.permission === 'granted') fresh.slice(0, 3).forEach(item => new Notification('Follow-up vencido · CRM Cadena', { body: item.title }))
        const next = [...sent, ...fresh.map(x => x.id)].slice(-100)
        localStorage.setItem(key, JSON.stringify(next))
      }
    }
    check(); timer = setInterval(check, 30000)
    return () => { mounted = false; if (timer) clearInterval(timer) }
  }, [supabase])
  useEffect(() => { if (notice.length && 'Notification' in window && Notification.permission === 'default') Notification.requestPermission().catch(() => {}) }, [notice.length])
