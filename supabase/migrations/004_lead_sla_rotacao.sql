...
begin
  if tg_op = 'INSERT' then
    ...
    new.assigned_at = now(); new.first_contact_at = null; new.last_contact_at = null; new.sla_last_reassigned_at = null;
  end if;
  return new;
end;
$$;
...
begin
  if new.tipo in ('ligacao', 'whatsapp', 'visita') then
    update public.leads set first_contact_at = coalesce(first_contact_at, new.created_at), last_contact_at = new.created_at, last_activity_at = new.created_at where id = new.lead_id;
  else
...
returns trigger language plpgsql security definer set search_path = public as $$
declare v_days numeric; v_role text;
begin
...
declare
  v_enabled boolean; v_grace integer; v_inactivity numeric; v_lead record; v_next uuid; v_current_name text; v_moved integer := 0;
begin
  -- Executável somente pelo service_role do servidor ou pelo usuário de sessão do pg_cron.
  if coalesce(auth.role(), '') <> 'service_role' and session_user <> 'postgres' then
    raise exception 'Rotina automática de SLA deve ser executada pelo servidor';
  end if;

  select enabled, initial_contact_grace_minutes, max_inactivity_days into v_enabled, v_grace, v_inactivity from public.lead_sla_settings where id = true;
  ...
  for v_lead in
    ...
    from public.leads l left join public.profiles p on p.id = l.responsavel_id
    where l.deleted_at is null and l.responsavel_id is not null and l.status not in ('ganho', 'perdido')
      and ((l.first_contact_at is null and now() >= coalesce(l.assigned_at, l.created_at) + (v_grace * interval '1 minute'))
        or (l.first_contact_at is not null and now() >= coalesce(l.last_contact_at, l.first_contact_at) + (v_inactivity * interval '1 day')))
    order by coalesce(l.assigned_at, l.created_at)
    for update of l skip locked
  loop
    -- Próximo corretor da lista: ordem alfabética, voltando ao primeiro quando chegar ao fim.
    select p2.id into v_next from public.profiles p2
    where p2.role = 'corretor' and p2.id <> v_lead.responsavel_id
      and (lower(p2.nome) > lower(coalesce(v_lead.responsavel_nome, '')) or not exists (
        select 1 from public.profiles p3 where p3.role = 'corretor' and p3.id <> v_lead.responsavel_id and lower(p3.nome) > lower(coalesce(v_lead.responsavel_nome, ''))))
    order by case when lower(p2.nome) > lower(coalesce(v_lead.responsavel_nome, '')) then 0 else 1 end, lower(p2.nome), p2.id limit 1;
    ...
    values (v_lead.id, null, 'sla_reatribuido', 'responsavel_id', v_lead.responsavel_id::text, v_next::text,
    ...
    insert into public.activities (lead_id, user_id, tipo, descricao) values (v_lead.id, null, 'observacao', 'Lead reatribuído automaticamente pelo SLA de atendimento.');
    v_moved := v_moved + 1;
  end loop;
  return v_moved;
end;
