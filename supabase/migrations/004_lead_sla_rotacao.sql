...
insert into public.lead_sla_settings (id)
values (true)
...
create policy "lead sla: authenticated read"
  on public.lead_sla_settings for select to authenticated using (true);

drop policy if exists "lead sla: managers write" on public.lead_sla_settings;
create policy "lead sla: managers write"
  on public.lead_sla_settings for all to authenticated
...
-- Sempre que o responsável mudar, o relógio de primeiro contato reinicia.
...
create trigger leads_prepare_assignment_sla
  before insert or update of responsavel_id on public.leads
  for each row execute function public.prepare_lead_assignment_sla();

-- Contato real registrado no CRM reinicia o relógio de atendimento.
...
create trigger activities_register_lead_contact_sla
  after insert on public.activities
  for each row execute function public.register_lead_contact_sla();

-- Limita a criação/alteração de follow-ups além do prazo configurado.
...
declare
  ...
  v_role text;
begin
  select max_followup_days into v_days
  from public.lead_sla_settings
  where id = true;

  v_role := public.current_user_role();

  if v_days is not null
     and new.due_at > now() + (v_days * interval '1 day')
     and v_role not in ('admin', 'gerente') then
...
create trigger lead_followups_enforce_sla
  before insert or update of due_at on public.lead_followups
  for each row execute function public.enforce_lead_followup_sla();

-- Processa os leads vencidos. Pode ser chamado pelo cron do Vercel.
...
declare
  ...
  v_moved integer := 0;
begin
  -- Só gestão ou service_role pode executar a rotina automática.
  if coalesce(auth.role(), '') <> 'service_role'
     and public.current_user_role() not in ('admin', 'gerente') then
    raise exception 'Sem permissão para processar SLA de leads';
  end if;

  select enabled, initial_contact_grace_minutes, max_inactivity_days
    into v_enabled, v_grace, v_inactivity
  from public.lead_sla_settings
  where id = true;
  ...
  for v_lead in
    select l.id, l.nome, l.responsavel_id, l.assigned_at, l.first_contact_at, l.last_contact_at,
    ...
    where l.deleted_at is null
      ...
      and (
        (l.first_contact_at is null
          and now() >= coalesce(l.assigned_at, l.created_at) + (v_grace * interval '1 minute'))
        or
        (l.first_contact_at is not null
          and now() >= coalesce(l.last_contact_at, l.first_contact_at) + (v_inactivity * interval '1 day'))
      )
    order by coalesce(l.assigned_at, l.created_at)
    for update of l skip locked
  loop
    -- Próximo corretor: ordem alfabética, com volta ao início da lista.
    ...
    order by
      case when lower(p2.nome) > lower(coalesce(v_lead.responsavel_nome, '')) then 0 else 1 end,
      lower(p2.nome), p2.id
    limit 1;
    ...
    update public.leads
    set responsavel_id = v_next,
        sla_last_reassigned_at = now()
    where id = v_lead.id;

    insert into public.lead_audit_log (
      lead_id, actor_id, action, field_name, old_value, new_value, metadata
    ) values (
      v_lead.id,
      null,
      'sla_reatribuido',
      'responsavel_id',
      v_lead.responsavel_id::text,
      v_next::text,
      jsonb_build_object(
        'motivo', case when v_lead.first_contact_at is null then 'primeiro_contato_expirado' else 'inatividade_expirada' end,
        'corretor_anterior', v_current_name,
        'executado_em', now()
      )
    );
    ...
    values (
      ...
      null,
      'observacao',
      'Lead reatribuído automaticamente pelo SLA de atendimento.'
    );
...
grant execute on function public.process_lead_sla() to authenticated;
